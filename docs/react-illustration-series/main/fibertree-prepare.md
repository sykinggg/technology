...
<img :src="$withBase('/assets/react-illustration-series/code2dom.png')" alt="demo" />
...
<img :src="$withBase('/assets/react-illustration-series/fibertreecreate1-progress.png')" alt="demo" />

1. 构造完成并渲染, 切换`fiberRoot.current`指针, 使其继续指向当前界面对应的`fiber`树(原来代表界面的 fiber 树, 变成了内存中).

<img :src="$withBase('/assets/react-illustration-series/fibertreecreate2-complete.png')" alt="demo" />
