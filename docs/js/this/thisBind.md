...
创建函数bar()，并在它的内部手动调用foo.call(obj)，强制把foo的this绑定到了obj。这种方式让想起了**借用构造函数继承**，没看过的可以点击查看
...
* 包括**内置对象函数**（比如`Number(..)`）在内的所有函数都可以用`new`来调用，这种函数调用被称为构造函数调用。
...
* 2、取出第一个参数，就是要传入的构造函数。此外因为 shift 会修改原数组，所以 `arguments` 会被去除第一个参数
...
// 之所以使用null是因为在本例中并不关心硬绑定的this是什么
// 反正使用new时this会被修改
...
安全的做法就是传入一个特殊的对象（空对象），把this绑定到这个对象不会对的程序产生任何副作用。
