...
<img :src="$withBase('/assets/vue/event-loop.png')" alt="原理图">
...
在 Vue 源码 2.5+ 后，`nextTick` 的实现单独有一个 JS 文件来维护它，它的源码并不多，总共也就 100 多行。接下来来看一下它的实现，在 `src/core/util/next-tick.js` 中：
...
通过这一节对 `nextTick` 的分析，并结合上一节的 setter 分析，了解到数据的变化到 DOM 的重新渲染是一个异步过程，发生在下一个 tick。这就是平时在开发的过程中，比如从服务端接口去获取数据的时候，数据做了修改，如果的某些方法去依赖了数据修改后的 DOM 变化，就必须在 `nextTick` 后执行。比如下面的伪代码：
...
Vue.js 提供了 2 种调用 `nextTick` 的方式，一种是全局 API `Vue.nextTick`，一种是实例上的方法 `vm.$nextTick`，无论使用哪一种，最后都是调用 `next-tick.js` 中实现的 `nextTick` 方法。
