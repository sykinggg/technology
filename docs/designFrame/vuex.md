...
[vuex-analysis](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Flxchuan12%2Fvuex-analysis) 源码分析仓库`fork`一份或者直接克隆下来， `git clone https://github.com/lxchuan12/vuex-analysis.git`

> 其中文件夹vuex，是克隆官方的vuex仓库 dev分支。
> 截至（2019年11月），版本是v3.1.2，最后一次commit是ba2ff3a3，2019-11-11 11:51 Ben Hutton。
> 包含笔者的注释，便于理解。
...
本文主要就是通过`Shopping Cart`，(路径`vuex/examples/shopping-cart`)例子调试代码的。
...
> 介绍了`vuex`、`vue`源码调试方法和 `Vuex` 原理。并且详细介绍了 `Vuex.use` 安装和 `new Vuex.Store` 初始化、`Vuex.Store` 的全部`API`（如`dispatch`、`commit`等）的实现和辅助函数 `mapState`、`mapGetters`、 `mapActions`、`mapMutations createNamespacedHelpers`。

[在vuex-analysis源码仓库有注解](https://github.com/lxchuan12/vuex-analysis)
