...
首先执行`beginWork`进行节点操作，以及创建子节点，子节点会返回成为`next`，如果有`next`就返回。返回到`workLoop`之后，`workLoop`会判断是否过期之类的，如果都OK就会再次调用该方法。
...
function performUnitOfWork(workInProgress: Fiber): Fiber | null {
    ...
    if (__DEV__) {
        ...
        if (isReplayingFailedUnitOfWork) {
            ...
            // 当前正在重播失败的工作单元。 这应该是无法达到的，因为渲染阶段是幂等的，所以它应该再次抛出。 既然没有，请重新抛出原始错误，因此React的内部堆栈未对齐。
...
首先有一个判断，这个判断是提升性能非常重要的一点，如果符合这个条件那么说明这个节点以及他的子节点很可能在这次更新中都不需要再被计算，先来看一下内容
...
`hasLegacyContextChanged`这个判断是用来判断`context`的，这个判断代表着如果当前节点和他的父节点都没有老的`context api`，也就是`childContextType`那套的话，他就为`true`，不然不管他的父级的`context`内容是否有变化，他都为`false`。所以老的`context api`对性能的影响还是非常大的

TODO: context api

第二个判断是超时时间的判断，如果当前节点的`expirationTime`是`NoWork`那么说明他没有更新，也就不需要改动了，如果`expirationTime`大于当前渲染的超时时间，说明他的更新内容不是这次更新造成的，也可以忽略。

如果符合条件，所有组件都会执行`bailoutOnAlreadyFinishedWork`，但是对于一些可以提供`context`的组件，仍然要做一些`context`相关的操作
    ...
    while (true) {
        ...
        if ((workInProgress.effectTag & Incomplete) === NoEffect) {
            ...
            if (next !== null) {
                ...
                // back here again.
                // 如果完成这项工作后又产生了新工作，请继续执行下一步。 会再次回到这里。
                return next;
            }
            ...
            if (siblingFiber !== null) {
                ...
                // 如果在该returnFiber中还有更多工作要做，请接下来执行。
                return siblingFiber;
            } else if (returnFiber !== null) {
                // If there's no more work in this returnFiber. Complete the returnFiber.
                // 如果此returnFiber中没有更多工作了。 完成returnFiber。
