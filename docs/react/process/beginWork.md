上一节了解到`render阶段`的工作可以分为“递”阶段和“归”阶段。其中“递”阶段会执行`beginWork`，“归”阶段会执行`completeWork`。这一节看看“递”阶段的`beginWork`方法究竟做了什么。
...
从上一节已经知道，`beginWork`的工作是传入`当前Fiber节点`，创建`子Fiber节点`，从传参来看看具体是如何做的。
...
从[双缓存机制一节](./doubleBuffer.html)知道，除[`rootFiber`](./doubleBuffer.md#mount%E6%97%B6)以外， 组件`mount`时，由于是首次渲染，是不存在当前组件对应的`Fiber节点`在上一次更新时的`Fiber节点`，即`mount`时`current === null`。
...
对于常见的组件类型，如（`FunctionComponent`/`ClassComponent`/`HostComponent`），最终会进入[reconcileChildren](https://github.com/facebook/react/blob/1fb18e22ae66fdb1dc127347e169e73948778e5a/packages/react-reconciler/src/ReactFiberBeginWork.new.js#L233)方法。
...
知道，`render阶段`的工作是在内存中进行，当工作结束后会通知`Renderer`需要执行的`DOM`操作。要执行`DOM`操作的具体类型就保存在`fiber.effectTag`中。
...
知道，`mount`时，`fiber.stateNode === null`，且在`reconcileChildren`中调用的`mountChildFibers`不会为`Fiber节点`赋值`effectTag`。那么首屏渲染如何完成呢？

针对第一个问题，`fiber.stateNode`会在`completeWork`中创建，会在下一节介绍。
...
> 为什么`rootFiber`节点存在`current`（即`rootFiber.alternate`），在[双缓存机制一节mount时的第二步](./doubleBuffer.html)已经讲过
...
[执行流程](https://codesandbox.io/s/fiber-root-hxvml)
...
<img :src="$withBase('/assets/react/beginWork.png')" alt="beginWork流程图">
