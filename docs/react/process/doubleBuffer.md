通过上一节的学习，了解了`Fiber`是什么，知道`Fiber节点`可以保存对应的`DOM节点`。
...
当用`canvas`绘制动画，每一帧绘制前都会调用`ctx.clearRect`清除上一帧的画面。
...
接下来以具体例子讲解`mount时`、`update时`的构建/替换流程。
...
之所以要区分`fiberRootNode`与`rootFiber`，是因为在应用中可以多次调用`ReactDOM.render`渲染不同的组件树，他们会拥有不同的`rootFiber`。但是整个应用的根节点只有一个，那就是`fiberRootNode`。
...
1. 接下来点击`p节点`触发状态改变，这会开启一次新的`render阶段`并构建一棵新的`workInProgress Fiber 树`。
...
[demo](https://codesandbox.io/s/fiber-switch-forked-qbn9r)
