在[新的React架构一节](/react/preparation/newConstructure.md)中，提到的**虚拟DOM**在`React`中有个正式的称呼——`Fiber`。在之后的学习中，会逐渐用`Fiber`来取代**React16虚拟DOM**这一称呼。

接下来让了解下`Fiber`因何而来？他的作用是什么？
...
你可以从这里看到[Fiber节点的属性定义](https://github.com/facebook/react/blob/1fb18e22ae66fdb1dc127347e169e73948778e5a/packages/react-reconciler/src/ReactFiber.new.js#L117)。虽然属性很多，但可以按三层含义将他们分类来看
...
作为动态的工作单元，`Fiber`中如下参数保存了本次更新相关的信息，会在后续的更新流程中使用到具体属性时再详细介绍
...
如果你的源码中`fiber.expirationTime`仍存在，请参照[调试源码](react/preparation/source.md)章节获取最新代码。


## 总结

本节了解了`Fiber`的起源与架构，其中`Fiber节点`可以构成`Fiber树`。那么`Fiber树`和页面呈现的`DOM树`有什么关系，`React`又是如何更新`DOM`的呢？
