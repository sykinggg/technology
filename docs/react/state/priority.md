...
优先级最终会反映到`update.lane`变量上。当前只需要知道这个变量能够区分`Update`的优先级。

接下来通过一个例子结合上一节介绍的`Update`相关字段讲解优先级如何决定更新的顺序。
...
可以看见，`u2`对应的更新执行了两次，相应的`render阶段`的生命周期勾子`componentWillXXX`也会触发两次。这也是为什么这些勾子会被标记为`unsafe_`。

## 如何保证状态正确

现在基本掌握了`updateQueue`的工作流程。还有两个疑问：
...
通过以上例子可以发现，`React`保证最终的状态一定和用户触发的`交互`一致，但是中间过程`状态`可能由于设备不同而不同。

details 高优先级任务打断低优先级任务Demo

<!-- [关注公众号](../me.html)，后台回复**815**获得在线Demo地址 -->

[多重异步更改](https://codesandbox.io/s/epic-borg-kx7jn)
